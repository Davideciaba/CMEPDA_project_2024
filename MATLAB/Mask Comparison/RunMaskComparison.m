...
%   Family-Wise Error topology corrections (p < 0.05) are applied on the estimated 
...
meanStats = meanMask.getMaskStats;
...
consensusStats = consensusMask.getMaskStats;
...
ridgMeanStats = ridgMeanMask.getMaskStats;
...
ridgTpmStats = ridgTpmMask.getMaskStats;
...
% Extract the corrected map based on Mean Mask (Family-Wise Error at p < 0.05)
[meanFweMap, meanThresh] = vbmModel.getCorrectedMap(meanResultsDir, contrastName, pValue, correctionMode);
...
% Extract the corrected map based on TPM Mask (Family-Wise Error at p < 0.05)
[tpmFweMap, tpmThresh] = vbmModel.getCorrectedMap(tpmResultsDir, contrastName, pValue, correctionMode);

% Calculate similarity metrics on the corrected maps
meanFweMapName = 'Mean FWE Map';
tpmFweMapName = 'TPM FWE Map';
simMetrics = vbmModel.evaluateMapSimilarity(meanFweMap, tpmFweMap, meanFweMapName, tpmFweMapName);
...
renderer.plotStatisticalOverlay(meanFweMap, meanThresh, CTRL117Volume, affineMat,...
        sliceConfig, contrastName, correctionMode, pValue, meanFweMapName, figMeanFwe);
...
renderer.plotStatisticalOverlay(tpmFweMap, tpmThresh, CTRL117Volume, affineMat,...
        sliceConfig, contrastName, correctionMode, pValue, tpmFweMapName, figTpmFwe);
...
renderer.plotCompositeOverlap(meanFweMap, tpmFweMap, CTRL117Volume, affineMat,...
        sliceConfig, meanFweMapName, tpmFweMapName);
