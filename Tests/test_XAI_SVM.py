...
Employs flat direct imports for the XAI module and cross-package imports for the Logger.
"""
import unittest
from unittest.mock import patch, MagicMock
import numpy as np

# Importazione locale (stessa cartella XAI)
from Python.XAI.XAI_SVM import SVMAnalyticalXAI

# Importazione cross-directory (dalla cartella Models)
...
class TestSVMAnalyticalXAI(unittest.TestCase):
    ...
    def test_compute_haufe_transform_math(self):
        ...
        with self.logger.context(Task="Haufe_Math"):
    ...
    def test_compute_gaonkar_maps_math(self):
        """Validates Gaonkar's non-parametric sample-to-voxel variance projection."""
        with self.logger.context(Task="Gaonkar_Math"):
            N_SAMPLES, N_FEATURES = 8, 50
            np.random.seed(42)
            ...
            z_map, p_map = self.xai_engine.compute_gaonkar_maps(mock_model, X_train, y_train, C=1.0)

            self.assertEqual(z_map.shape, (N_FEATURES,))
            self.assertEqual(p_map.shape, (N_FEATURES,))
            self.assertTrue(np.all((p_map >= 0.0) & (p_map <= 1.0)))
    ...
    # I target dei patch puntano al namespace locale "XAI_SVM"
