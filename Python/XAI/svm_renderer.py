...
class SVMRenderer:
    """
    Renderer optimized for SVM maps (Gaonkar Z-maps or Haufe Activation Maps).
    """
    ...
    def plot_svm_map(self, map_path: str, bg_path: str, out_fig_path: str, 
        ...
        svm_vol = nib.load(map_path).get_fdata()
        ...
        for idx, (z_vox, z_mm) in enumerate(zip(z_voxels, z_mms)):
            ...
            slice_stats = np.rot90(svm_vol[:, :, z_vox])
            
            ax.imshow(slice_bg, cmap='gray')
            
            overlay_colors = cmap(norm(slice_stats))
            alpha_layer = (slice_stats >= threshold).astype(float) * 0.75
        ...
        cb.set_label('Activation Score', color='white', fontweight='bold')
        ...
        plt.close(fig)
        self.logger.success(f"SVM Map saved to: {out_fig_path}")